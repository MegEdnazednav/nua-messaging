class PrescriptionJob < ActiveJob::Base
  ...
  def perform(user)
    begin
      ...
      raise Rails.logger.error "[PrescriptionJob] Could not finish the request"
    end
  end
  ...
  def send_message_to_admin
    ...
    raise Rails.logger.error "[PrescriptionJob] Message not created"
  end

  def send_api_payment_request
    PaymentProviderFactory.provider.debit_card(user)
  rescue => e
    raise Rails.logger.error "[PrescriptionJob] Could not connect to API"
  end

  def create_payment_record
    @payment = Payment.create(user_id: user.id)
  rescue
    raise Rails.logger.error "[PrescriptionJob] Payment not created"
  end
